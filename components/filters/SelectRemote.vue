...
export default {
  ...
  data() {
    ...
    return {
      ...
      loading: false,
      fetchDebounce: null,
      selectValue: /^-?\d+$/.test(defaultValue)
      ...
      handler(query) {
        if (this.router) {
          if (/^-?\d+$/.test(query[this.query])) {
            ...
            this.selectValue = _map(_split(query[this.query], ","), (value) =>
              parseInt(value, 10)
            );
          } else {
  ...
  methods: {
    ...
    handleSelect(value) {
      ...
      const queryValue =
        Array.isArray(value) && value.length > 0 ? _join(value, ",") : value;
      ...
      if (this.router) {
        ...
        this.customHandler(
          value,
          this.selectionItems.find(
            (option) => _get(option, this.optionValue) === value
          )
        );
      }
    },
